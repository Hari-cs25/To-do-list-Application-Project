...
import { formContainer2 } from "./forme/Forme2.js";

// LOCAL STORAGE

export let currentProjectIndex;
export const setCurrentProjectIndex = (num) => {
...
currentProjectIndex =
  JSON.parse(localStorage.getItem("todoApp.CurentProjectIndex")) ?? 0;
  ...
  contentSpace.appendChild(
    createTodoCard(
      addTodo(
        ...
        complete.value,
        date.value,
        priarityoption.value,
      ),
    ),
  );
  console.log(
    "priority that is added in the todos of the current project ->",
    project[currentProjectIndex].todos[
      project[currentProjectIndex].todos.length - 1
    ].priority,
  );
  saveDatas();
  renderTodos(project[currentProjectIndex]);
